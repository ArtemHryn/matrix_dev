...
const AgeRowElement = ({ index, currentArcane, age, arcane, additional, date, isGenerated }) => {
  ...
  return (
    ...
    >
      <AgeElement borderBottom={index === 15 && ['none', 'none', 'none']}>{age}</AgeElement>{' '}
      <AgeElement flex="3" borderBottom={index === 15 && ['none', 'none', 'none']}>{`${arcane} - ${
        additional[index].arcane
      } - ${checkNum(arcane + additional[index].arcane)}`}</AgeElement>
      {!isGenerated && (
        <OverlayPanel ref={op}>
          <PeriodTip>
